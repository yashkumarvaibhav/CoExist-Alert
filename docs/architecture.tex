...
Field sensors are simulated in this proof of concept and are labeled that way in the UI and diagrams. Demo controls are a user-facing surface: they can trigger the ingest API for rehearsal, but they are not a field sensor node. Every user-facing surface sits behind sign-in with role-based access; the demo accounts are deliberately open one-click logins so reviewers can explore each role.
\end{decisionbox}
...
\begin{center}
\makebox[\textwidth][c]{\includegraphics[width=1.12\textwidth,height=0.82\textheight,keepaspectratio]{diagrams/architecture-system.pdf}}
\end{center}
...
\begin{center}
\makebox[\linewidth][c]{\includegraphics[width=1.12\linewidth,height=0.82\textheight,keepaspectratio]{diagrams/architecture-sequence.pdf}}
\end{center}
\end{landscape}
