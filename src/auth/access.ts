 ...
 */
export type RouteAccess = { kind: "public" } | { kind: "protected"; roles: UserRole[] };
...
export function routeAccess(pathname: string): RouteAccess {
  ...
  if (pathname === "/demo") return { kind: "protected", roles: ["admin"] };
  ...
  if (pathname.startsWith("/api/ingest")) return { kind: "protected", roles: ["admin"] };
  if (pathname.startsWith("/api/demo")) return { kind: "protected", roles: ["admin"] };
