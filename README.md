...
| **Observe** | ThousandEyes, Splunk | Agent-to-server tests per node; alert on loss/latency; Splunk drives analytics | Heartbeat pipeline models ThousandEyes tests: missed beats ⇒ degraded/offline with the same alarm semantics; analytics engine is Splunk-style aggregation | Health board labelled with its reliability model |
