...
| **Secure** | Duo / Secure Access / Umbrella | MFA on the command console; zero-trust responder access; DNS-layer protection for field gateways | **Real auth + RBAC** — signed-in accounts with scrypt-hashed passwords and HMAC session cookies; middleware gates every console and mutating API by role (a guard reaches only their console; field-driving controls are admin-only). Duo MFA is the production step-up gate for the operations console when configured. One-click demo accounts in the landing-page sign-in modal keep it explorable | `LIVE` (auth enforced) — demo credentials shown in the sign-in modal |
